...
Page({
  ...
  onLoad: function(options) {
    ...
    that.requestAddress()
    WxNotificationCenter.addNotification(getApp().shuaXin.youHuiQuan, that.getYouHuiQuan, that)
  },
   ...
   */
  onUnload: function() {
    WxNotificationCenter.removeNotification(getApp().shuaXin.youHuiQuan, this)
  },
      ...
      success: function(res) {
        ...
        if (queRenDingDan.state == 1) {
          ...
          that.setData({
            ...
            price: (price + that.data.yunFeiPrice).toFixed(2),
            priceSP: price,
          })
          that.getYouHuiQuan()
        } else { //请求失败
  ...
  getYouHuiQuan() {
    ...
    wx.request({
      ...
      success: function(res) {
        ...
        if (res.data.state == 1) {
          var price = 0;
          price = (that.data.priceSP + that.data.yunFeiPrice - res.data.data.coin).toFixed(2)
          if(res.data.data){
            res.data.data.coin = res.data.data.coin.toFixed(2)
            res.data.data.usemin = res.data.data.usemin.toFixed(2)
          }
          wx.hideLoading()
          that.setData({
            isfail: false,
